...
#' @export
constrained_sgt <- function(link='identity', link_sigma='log', link_qinv='log', link_pq='logm1', link_lambdap1half='logit'){
  brms::custom_family(
    name='constrained_sgt',
    dpars=c('mu', 'sigma', 'lambdap1half', 'qinv', 'pq'),
    lb = c(NA, 0, 0, 0, 1),
    ub = c(NA, NA, 1, NA, NA),
    links=c(mu=link, sigma=link_sigma, lambdap1half=link_lambdap1half, q=link_qinv, pq=link_pq),
...
constrained_sgt_default_prior <- function(params = 'all', exclude=FALSE){
  default_priors <-
    list(
      ...
      brms::prior(inv_gamma(3, .1), class='qinv', lb=0),
      brms::prior(gamma(3, .1), class='pq', lb=1)
    )

  which <- c('sigma', 'lambdap1half', 'qinv', 'pq')
  if ('all' %in% params) {
...
log_lik_constrained_sgt <- function(i, prep) {
  ...
  qinv <- brms::get_dpar(prep, "qinv", i=i)
  q <- 1/qinv
